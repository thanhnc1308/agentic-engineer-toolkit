...
An agentic engineering toolkit for full-stack software development across multiple AI coding agents.
...
This repository supports both Codex and Claude Code with the same shared workspace.
...
> `.workspace/` is the canonical dev environment (skills, agents, commands). Keep this file aligned with `CLAUDE.md` so both agents follow the same conventions.
...
- `.claude-plugin/marketplace.json` - Plugin registry for Claude marketplace distribution
