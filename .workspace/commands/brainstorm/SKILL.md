...
2. **Load Jira context when provided** — If the request includes a Jira issue key (for example `PROJ-123`) or Jira URL, retrieve the ticket with Atlassian MCP before asking discovery questions:
   - Use `mcp__atlassian__searchAtlassian` with the issue key (or URL text) to locate the issue.
   - Use `mcp__atlassian__fetchAtlassian` on the returned Jira ARI to read full issue details.
   - If ARI lookup is unavailable, use `mcp__atlassian__getJiraIssue` with the issue key.
   - Extract and confirm: summary, description, status, priority, assignee/reporter, linked issues, and acceptance criteria (if present).
   - If access fails or the issue cannot be found, tell the user exactly what failed and continue discovery using the user's text as the source of truth.
3. **Challenge assumptions** — Before diving in, ask whether this is the right problem to solve. Is there a simpler framing? Could doing nothing be acceptable?
4. **Gather** — Always confirm understanding with the user, even if the request seems clear. Prefer multiple choice questions when possible:
...
5. **Synthesize** — From the request, Jira context (if any), and user answers, produce:
