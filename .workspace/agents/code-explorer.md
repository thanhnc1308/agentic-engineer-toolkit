...
- **If no repomix snapshot is provided**, generate one by running `mkdir -p ~/.agent-workspace/repomix && SERVICE_OR_REPO_NAME="${SERVICE_NAME:-$(basename "$(git rev-parse --show-toplevel)")}" && repomix --style markdown -i "node_modules/**,dist/**,coverage/**,vendor/**" -o ~/.agent-workspace/repomix/${SERVICE_OR_REPO_NAME}-repomix-output.md` in the target project directory. Always ignore `node_modules/**,dist/**,coverage/**,vendor/**` by default. If the user wants to ignore comments, add `--remove-comments` (e.g., `SERVICE_OR_REPO_NAME="${SERVICE_NAME:-$(basename "$(git rev-parse --show-toplevel)")}" && repomix --include "src/**/*.ts" -i "node_modules/**,dist/**,coverage/**,vendor/**,**/*.test.ts" --style markdown --remove-comments -o ~/.agent-workspace/repomix/${SERVICE_OR_REPO_NAME}-repomix-output.md`). Then read the generated file as your primary orientation source. If `repomix` is not installed, fall back to manual exploration below.
