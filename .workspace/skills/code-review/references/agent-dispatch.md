...
ALWAYS dispatch:
  ...
  - security-scanner (subagent_type: security-scanner)
  - performance-reviewer (subagent_type: performance-reviewer)

IF diff contains error handling patterns (catch|try|except|\.catch\(|on_error|fallback|rescue):
  - silent-failure-hunter (subagent_type: silent-failure-hunter)
